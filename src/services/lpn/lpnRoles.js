...
export const PORTAL_TO_FIELD = Object.freeze({
  ...
  /**
   * ★★★ وحداتُ الميدان الثلاث ‹JR-701› — خُرّطت بعد أن قاسها الحارسُ أدناه.
   *
   * كانت **غائبةً عن هذه الخريطة وحدها**: البوّابة تفتح لها شاشاتِ الاستلام
   * والتخزين والتحضير، و`firestore.rules` تعدّها فاعلًا مخزنيًّا — فكانت تمرّ
   * في `uiGate` بـ`{known:false}`، أي **لأنّها مجهولةٌ لا لأنّها مأذونة**.
   * وفرقُ الاثنين كلُّ شيء: المجهولُ تُخفي عنه الشاشةُ رسالةَ المنع، ومن
   * يقرأ هذه الخريطةَ حكمًا (`barcodeKinds` · `stageOwners`) **يمنعه** لأنّه
   * لا يجد له عمليّة — فيُمنع في الشاشة من يسمح له الخادم.
   *
   * وكلُّ وحدةٍ **دورٌ ميدانيٌّ واحدٌ باسمها**: لا توسيعَ هنا بل تسميةُ ما
   * كان قائمًا في البوّابة والقاعدة أصلًا.
   */
  receiving_unit: ['RECEIVER'],
  putaway_unit: ['PUTAWAY'],
  picking_unit: ['PICKER'],
});
...
/**
 * ★★ عمليّةُ شاشة «مهامي» ‹JR-701› — تُصدَّر كي لا تُكتب مرّتين فتنحرف.
 *
 * الشاشةُ تُنفّذ مهامَّ مناولةٍ على مستوى السطر، وهي نوعان في `laborModel`:
 * `putaway` و`pick`. فأيُّهما عمليّتُها في المصفوفة؟
 *
 * ★★★ **`PUTAWAY` — لأنّ `PICK` تُقصي صاحبَ الشاشة نفسَه.** مشرفُ المناولة
 * (`labor_supervisor`) يملك `PUTAWAY` و`LOADER` ولا يملك `PICKER`؛ فسؤالُ
 * المصفوفة عن `PICK` يُغلق «مهامي» في وجه من أوكلها له المالكُ في ق‑أ —
 * وتبقى الشاشةُ للمديرَين وحدهما. و`PUTAWAY` هي العمليّةُ التي يملكها
 * كتّابُ `labor_tasks` الثلاثة جميعًا، فهي **القاسمُ الصادق**.
 *
 * ⚠️ ولا تكفي وحدَها: تُقرن دائمًا بـ`collectionWriteProblem(role,'labor_tasks')`
 * لأنّ `uiGate` تمرّر المجهولَ عمدًا. اقرأ تعليقَها أدناه.
 */
export const MY_TASKS_OP = 'PUTAWAY';
