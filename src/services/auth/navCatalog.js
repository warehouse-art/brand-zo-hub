...
/**
 * ⚠️ **كلُّ بندٍ يُكتب في سطرٍ واحد** بـ`path` و`label` و`roles` صريحةً
 * بينها. فقارئُ الدليل (`scripts/build-usage-guide.mjs` · `readNav`) يقرأ
 * الملفَّ **سطرًا سطرًا**، وبندٌ موزَّعٌ على أسطرٍ أو `roles` بمتغيّرٍ باسمه
 * **لا يراه** — فتسقط الشاشةُ من دليل الاستخدام صامتةً، وهي قاعدةُ المالك
 * «لا ميزةَ بلا شرح». (كُشف عند دمج م٧ 2026-08-28.)
 */
...
export const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/dashboard/barcode-center', label: 'مركز الباركود', icon: 'clipboardList', roles: ['admin', 'warehouse_manager', 'storekeeper', 'inventory_auditor'] },
    ],
  },
  {
    /**
    ...
    items: [
      ...
      // أوّلًا.
      //
      // ═══ ‹JR-701› سحبٌ مقصود: أربعةُ أدوارٍ خرجت — والسببُ مكتوبٌ هنا ═══
      // ⚠️ **سببُ تحديث `navBaseline.json`** (شُغّل `nav-baseline.mjs --write`
      // عمدًا لأجل هذا السطر وحده):
      //
      // كانت مفتوحةً لسبعةٍ، ومجموعةُ `labor_tasks` في `firestore.rules`
      // محكومةٌ بـ`isLaborWriter()` — و**هي ثلاثةُ أدوارٍ لا سبعة**. فأمينُ
      // المخزن وضابطُ البوّابة ووحدتا التخزين والتحضير كانوا يفتحون الشاشة،
      // ويمشون إلى الرفّ، فيرتدّ أوّلُ مسحةٍ بـ`permission-denied` **في منتصف
      // المهمّة** بلا سببٍ يفهمونه. وأسوأُ منعٍ ما وقع بعد أن يبدأ العمل.
      //
      // وقرارُ المالك (ق‑أ): «أغلقها على المشرفين الآن» — لا نشرَ قاعدة.
      // والعاملُ الميدانيّ **لم يفقد عملًا**: شاشاتُ الطبالي (`lpn-*`) ولوحةُ
      // الخانة وخطّةُ السحب باقيةٌ له، وهي مسموحةٌ له في الخادم أصلًا. فما
      // سُحب بابٌ كان يُفضي إلى رفضٍ حتمًا — لا قدرةٌ كان يملكها.
      { path: '/dashboard/my-tasks', label: 'مهامي — تنفيذ التخزين والسحب', icon: 'checkSquare', roles: ['admin', 'warehouse_manager', 'labor_supervisor'] },
  ...
  },
  {
    ...
    items: [
      ...
      // ‹BFP-SCM-MAN-2026-053› الدليلُ الحاكم لسلاسل الإمداد — يُولَّد من
      // `src/data/scm-manual.json` لا يُكتب بيد، فلا تفترق نسختانِ منه.
      //
      // ⚠️ **حدٌّ معلَنٌ لا مخبوء:** موضعُه هنا يُوصله إلى **١٠ أدوارٍ من ١٩**
      // (من يرى «مركز التقارير»). والدليلُ يُلزم كلَّ دورٍ لا عُشرَه، لكنّ
      // إيصالَه إلى الباقين يحتاج تعديلَ `ROLE_NAV` — وهو من شأن طبقة
      // الأدوار لا من شأن إضافةِ مرجع. يُنجَز حين تُعتمد خطّةُ الأدوار.
      { path: '/دليل-السلاسل-والإمداد.html', label: 'دليل السلاسل والإمداد ‹الحاكم›', icon: 'bookOpen', external: true },
